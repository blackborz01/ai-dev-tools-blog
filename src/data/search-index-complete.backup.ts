...
// Import all MCP data sources
let mcpData: any[] = []
try {
  const mcps = require('./mcps')
  if (mcps.mcpServers) mcpData.push(...mcps.mcpServers)
} catch (e) {}

try {
  const mcpsComplete = require('./mcps-complete')
  if (mcpsComplete.mcpServers) mcpData.push(...mcpsComplete.mcpServers)
} catch (e) {}

try {
  const mcpsUpdated = require('./mcps-updated')
  if (mcpsUpdated.mcpServers) mcpData.push(...mcpsUpdated.mcpServers)
} catch (e) {}

// Import AI tools data
let aiToolsData: any[] = []
try {
  const aiToolsComplete = require('./ai-tools-complete')
  if (aiToolsComplete.aiTools) aiToolsData.push(...aiToolsComplete.aiTools)
  if (aiToolsComplete.tools) aiToolsData.push(...aiToolsComplete.tools)
} catch (e) {}

try {
  const aiToolsMega = require('./ai-tools-mega')
  if (aiToolsMega.aiTools) aiToolsData.push(...aiToolsMega.aiTools)
  if (aiToolsMega.tools) aiToolsData.push(...aiToolsMega.tools)
} catch (e) {}
...
export function getAllSearchableItems() {
  ...
  // Step 1: Add hardcoded complete tools list (50 tools)
  COMPLETE_TOOLS_LIST.forEach(tool => {
  ...
  // Step 2: Add hardcoded complete MCP list (70+ servers)
  COMPLETE_MCP_LIST.forEach(mcp => {
  ...
  // Step 3: Add discovered tools from JSON
  ...
  // Step 4: Add discovered MCPs from JSON
  ...
  // Step 5: Add discovered APIs from JSON
  ...
  // Step 6: Add from TypeScript MCP files
  ...
  // Step 7: Add from AI tools TypeScript files
